export const PACKAGE_NAME = "@x500/db-algorand" as const;

/** Algorand stack Supabase — dedicated project URL (not shared with other chains). */
export function getAlgorandSupabaseUrl(): string | undefined {
  return (
    process.env.ALGORAND_SUPABASE_URL?.trim() ??
    process.env.SUPABASE_ALGORAND_URL?.trim()
  );
}

export function getAlgorandSupabaseServiceRoleKey(): string | undefined {
  return (
    process.env.ALGORAND_SUPABASE_SERVICE_ROLE_KEY?.trim() ??
    process.env.SUPABASE_ALGORAND_SERVICE_ROLE_KEY?.trim()
  );
}

export function getAlgorandSupabaseDbUrl(): string | undefined {
  return (
    process.env.ALGORAND_SUPABASE_DB_URL?.trim() ??
    process.env.SUPABASE_ALGORAND_DB_URL?.trim()
  );
}

export function requireAlgorandSupabaseConfig(): {
  url: string;
  serviceRoleKey: string;
} {
  const url = getAlgorandSupabaseUrl();
  const serviceRoleKey = getAlgorandSupabaseServiceRoleKey();
  if (!url || !serviceRoleKey) {
    throw new Error(
      "ALGORAND_SUPABASE_URL and ALGORAND_SUPABASE_SERVICE_ROLE_KEY are required " +
        "(use the Algorand Supabase project credentials)",
    );
  }
