...
**Local UI:** `pnpm dashboard:dev` (explorer + merchant registration) · `pnpm chat:dev` (insured Groq weather agent on port 3002).

Try the **chat demo** locally: a LangChain agent on Groq calls registered merchants in two modes — **successful response** (`http://127.0.0.1:8800`) and **SLA breach + refund** (`http://127.0.0.1:8801`).
...
|-------|----------------|---------------------|-----------|---------|
| Merchant onboarding | Register slug, origin URL, API price (microUSDC), SLA, pay-to address | `X500Registry.register_endpoint` | Merchant wallet (Pera / Defly via [dashboard](packages/dashboard)) | See [Registry app](https://lora.algokit.io/testnet/application/769438875) |
...
| **Merchant** | List a paid API, receive x402 USDC | Dashboard (`pnpm dashboard:dev`), own x402 server |
...
flowchart TB
  ...
  AD -->|"① insured fetch + escrow"| AGENT_PATH
  MER -->|"② register slug & SLA"| MERCHANT_PATH

  AGENT_PATH -->|"③ x402 USDC + upstream HTTP"| APIS
  AGENT_PATH -->|"④ call outcome"| SETTLE_PATH
  MERCHANT_PATH -->|"② on-chain registration"| CHAIN
  SETTLE_PATH -->|"⑤ settle_batch premium / refund"| CHAIN
```

**Reading the diagram:** agents enter through the SDK (①), merchants register via the dashboard (②), paid API traffic exits to upstream merchants (③), outcomes flow to the settler (④), and premiums/refunds land on Algorand (⑤).
...
| Dashboard | `pnpm dashboard:dev` | Registration + explorer |
| Chat | `pnpm chat:dev` | Groq insured weather demo |
...
```
┌─────────┐     ┌──────────────┐     ┌────────────────┐     ┌───────────┐
│ Agent   │────▶│ Market proxy │────▶│ Merchant x402  │────▶│ Facilitator│
│ + SDK   │     │ /v1/{slug}   │     │ origin URL     │     │ (USDC)    │
└─────────┘     └──────┬───────┘     └────────────────┘     └───────────┘
                         │
                         ▼
                  ┌──────────────┐     ┌──────────┐     ┌─────────────┐
                  │ wrapFetch    │────▶│ Supabase │────▶│ Settler     │
                  │ classify     │     │ jobs     │     │ settle_batch│
                  └──────────────┘     └──────────┘     └─────────────┘
```
...
The **chat UI** ([`chat/`](chat/)) runs the same pattern in the browser with fast vs SLA-breach merchant modes.
...
2. Open dashboard (`pnpm dashboard:dev`) → **Merchants → Register**.
...
End-to-end walkthrough using [`example/server`](example/server), [`example/agent`](example/agent), and optional [`chat`](chat/).
...
| Chat insured demo | **Shipped** | `pnpm chat:dev` |
...
- Registration dashboard with Pera / Defly
